...
@implementation OAButton_Download
...
- (void) emptyButton
{
    ...
    if(self.fillPercent<0.0){
        if(self.delegate && [self.delegate respondsToSelector:@selector(buttonIsEmpty:)]){
            [self performSelector:@selector(buttonEmpty) withObject:self afterDelay:0.5];
