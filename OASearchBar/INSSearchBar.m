...
- (id)initWithFrame:(CGRect)frame
{
    if ((self = [super initWithFrame:CGRectMake(CGRectGetMinX(frame), CGRectGetMinY(frame), CGRectGetWidth(frame), 34.0)]))
	{
        ...
        self.searchFrame.layer.borderWidth = 0;//0.5;
        ...
        self.searchField.textColor = kThemeColor;
        self.searchField.tintColor = kThemeColor;
		self.searchField.alpha = 0.0;
		self.searchField.delegate = self;
		
		[self.searchFrame addSubview:self.searchField];
		
		UIView *searchImageViewOnContainerView = [[UIView alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) - kINSSearchBarInset - kINSSearchBarImageSize - 7, (CGRectGetHeight(self.bounds) - kINSSearchBarImageSize) / 2, kINSSearchBarImageSize, kINSSearchBarImageSize)];
